...
const getListActiveWebHooks = async (
  headers: any,
  shop: string
): Promise<WebHook[]> => {
  try {
    ...
    if (webHooks?.length > 0) {
      ...
      const rmWebHooks = webHooks.filter((w) =>
        w.address.includes(process.env.WEBHOOK_TARGET_URL)
      );
...
const deleteAllWebHooks = async (headers, shop): Promise<SuccessResponse> => {
  try {
    ...
    if (list && list.length > 0) {
      ...
      return {
        success: true,
        message: "no webhooks registered, no deletion was needed",
      }; // return success, if no webhooks, no need to delete anything
  } catch (err) {
    console.log("deleteAllWebHooks err", err);
    return {
      success: false,
      message: "exception in deletion request:" + JSON.stringify(err),
    };
  }
...
const createWebHook = async (
  ...
  try {
    ...
    return webHook.id
      ...
      : { success: false, message: "webhook has no id" };
  } catch (err) {
    console.log("err", err);
...
async function handler(req, res) {
  ...
  if (!option) {
    ...
    res
      .status(200)
      .json({ success: false, message: "No option value in body" });
    return;
  }
  ...
  if (option === "list") {
    if (activeListAtStart && activeListAtStart.length > 1) {
      res
        ...
        .json({
          success: false,
          message: "more than 1 hook, should never happens",
        });
      return;
    }
    if (activeListAtStart && activeListAtStart.length === 1) {
      ...
      const option =
        activeListAtStart[0].topic === "orders/create"
          ? "auto_create"
          : "auto_fullfill";
  ...
  if (activeListAtStart && activeListAtStart.length > 0) {
    ...
    if (!successDeletion.success) {
      res
        .status(200)
        .json({ success: false, message: successDeletion.message });
      return;
    }
