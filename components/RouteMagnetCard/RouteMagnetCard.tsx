...
import {CircleTickMajor, ArrowLeftMinor, ChevronRightMinor, CirclePlusOutlineMinor, ChevronDownMinor, ArrowDownMinor} from '@shopify/polaris-icons';
...
// setOpenSection({...openSection, [order.statusAction.status]:{show:true}})
 ...
 return <p>{
  j.displayFulfillmentStatus === 'FULFILLED' ? 'Fulfilled status' : 'Unfulfilled Status'
  }</p>
}
...
const inWaitingQueueDetails = (j:JobOrder,isCurrentStatus:boolean) => {
  if(j?.job?.addedAt) {
    return (<React.Fragment>
         <TextStyle variation="subdued">{new Date(j?.job?.addedAt).toLocaleString() } </TextStyle>
     <div><TextStyle variation="strong"> Added to waiting queue by {j?.job?.addedByLabel ?? 'Shopify Admin' } </TextStyle></div>
     <div> <a href={`${process.env.NEXT_PUBLIC_RM_CLIENT_URL}/tabs/queue`} target="_blank">Waiting Queue</a></div>
    </React.Fragment>)
  }
...
const inPlannerBuilderDetails = (j:JobOrder,isCurrentStatus:boolean) => {
  if(j?.job?.pickedAt){
    return ( <React.Fragment>
      <TextStyle variation="subdued">{new Date(j?.job?.pickedAt).toLocaleString() } </TextStyle>
 ...
 const inItinerarySavedDetails = (j:JobOrder,isCurrentStatus:boolean) => {
  if(j?.job?.routeId) {
    return (<React.Fragment>       
     <div><TextStyle variation="strong"> itinerary saved by {j?.job?.pickedByLabel ?? 'user'} </TextStyle></div>
    </React.Fragment>)
  }

  return <p>Itinerary not saved</p>
 }
 const inItineraryAssignedDetails = (j:JobOrder,isCurrentStatus:boolean) => {
  if(j?.job?.step?.assignedAt) {
    return (<React.Fragment>
         <TextStyle variation="subdued">{new Date(j?.job?.step?.assignedAt).toLocaleString() } </TextStyle>
     <div><TextStyle variation="strong"> Assigned to {j?.job?.step?.assignedToLabel ?? 'user'} by {j?.job?.step?.assignedByLabel ?? 'Shopify Admin' } </TextStyle></div>
    </React.Fragment>)
  }
 ...
 const completedDetails = (j:JobOrder,isCurrentStatus:boolean) => {
  if(j?.job?.step?.completedAt) {
    return (<React.Fragment>
         <TextStyle variation="subdued">{new Date(j?.job?.step?.completedAt).toLocaleString() } </TextStyle>
     <div><TextStyle variation="strong"> Completed By: {j?.job?.step?.completedBy ?? 'user'} </TextStyle></div>
    </React.Fragment>)
  }
...
const onSectionClicked = (status:string) => {
  ...
  setOpenSection({
    ...openSection,[status]:{show: !openSection[status].show}
  })
}
      ...
      return <React.Fragment>
        ...
        { !futureStep &&
              <Stack.Item fill={false}>
                <Icon
                  source={openSection[status].show ? ChevronDownMinor : ChevronRightMinor}
                  color="base" />
                </Stack.Item>     
            }
        </Stack>
        ...
        <Stack>
          <Stack.Item>         
          {openSection[status].show && detailsMapping[status].action(o,i === index ? true : false)}       
          </Stack.Item>
        </Stack>
        <Stack>
          <Stack.Item>
