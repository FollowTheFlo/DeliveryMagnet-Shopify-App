...
const FulfillCard = (props: JobOrderProps) => {
  ...
  const displayCardSections = () => {
    return (
      <React.Fragment>
              ...
              return (
                <ResourceList.Item id={id} url={""}>
                  <Stack alignment="center">
                    <Stack.Item fill>
                      <Thumbnail source={image.originalSrc} alt={title} />
                    </Stack.Item>
                    <Stack.Item fill>
                    ...
                    <Stack.Item>
                      <Badge
                        status={
                          statusBadgeColorMapping[fulfillmentStatus] ?? "new"
                        }
                      >
                        {fulfillmentStatus}
                      </Badge>
        ...
        <Card.Section>
          Total: {currencyMapping[order.totalPriceSet.shopMoney.currencyCode]}
  ...
  return order.statusAction.action && !order.cancelledAt ? (
    <Card
      title="Items"
      primaryFooterAction={{
    ...
    <Card title="Items">{displayCardSections()}</Card>
  );
};
